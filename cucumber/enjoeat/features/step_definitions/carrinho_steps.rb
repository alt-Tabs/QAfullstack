...
Então("deve ser adicionado ao carrinho {int} unidade\\(s) deste item") do |quantidade|
...
Dado("que eu tenho os seguintes itens no carrinho:") do |table|
  @product_list = table.hashes
  #dynamic steps= adiciona um step bdd existente
  steps %{ 
...
Quando("eu removo todos os itens") do
  @product_list.each.with_index do |value, idx|
    @rest_page.cart.remove_item(idx)
  end
end

Quando("eu limpo o meu carrinho") do
  @rest_page.cart.clean_cart
end
